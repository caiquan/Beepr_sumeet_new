...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    ...
    CatalyzeQuery *query1 = [CatalyzeQuery queryWithClassName:PF_BLOCKED_CLASS_NAME];
    [query1 setQueryField:PF_BLOCKED_USER1];
    [query1 setQueryValue:user];
    [query1 setPageNumber:1];
    [query1 setPageSize:100];
    [query1 retrieveInBackgroundWithSuccess:^(NSArray *result){
        CatalyzeQuery *query2 = [CatalyzeQuery queryWithClassName:@"Users"];
        [query2 setPageNumber:1];
        [query2 setPageSize:100];
        [query2 retrieveInBackgroundWithSuccess:^(NSArray *_results){
            [users2 removeAllObjects];
            for(CatalyzeEntry * entry in _results){
                NSArray *emails = [[entry content] valueForKey:PF_USER_EMAILCOPY];
                NSArray *blocked = [(CatalyzeEntry *)[result firstObject] valueForKey:PF_BLOCKED_USER2];
                if(![[[entry content] valueForKey:PF_USER_OBJECTID] isEqualToString:user.usersId] && [emails containsObject:emails]
                   && ![blocked containsObject:[[entry content] valueForKey:PF_USER_OBJECTID]]){
                    [users2 addObject:entry];
                    [self removeUser:[[entry content] valueForKey:PF_USER_EMAILCOPY]];
                }
            }
            [self.tableView reloadData];
        } failure:^(NSDictionary* _results , int status, NSError *error){
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	{
        CatalyzeEntry *user = users2[indexPath.row];
        cell.textLabel.text = [[user content]valueForKey:PF_USER_FULLNAME];
        cell.detailTextLabel.text = [[user content] valueForKey:PF_USER_EMAILCOPY];
